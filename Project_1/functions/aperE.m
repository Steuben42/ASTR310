function [flx,err,Nsky,Nstarnet,n,Nskyerr,Nstarneterr]=aperE(im,col,row,rad1,rad2,ir1,ir2,or1,or2,Kccd,saturation)

%{
Original code by Professor Andrew Harris. Edited by Alyssa Pagan and Owen McWilliams
...
sky=median(im(ixsky)); Nsky = sky;            % sky value
pix=im(ixsrc)-sky; Nstarnet = sum(pix);       % source without sky
sig=sqrt(im(ixsrc)/Kccd);                     % photon noise per pixel
    Nstarneterr=sum(sig)/23509;               
